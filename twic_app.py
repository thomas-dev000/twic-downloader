...
import io,json,hashlib,zipfile,subprocess,sys,os,threading,logging,base64,platform
...
try:
    ...
    HAS_TK=False
try:
    import chess.pgn; import requests
    HAS_DEPS=True
except ImportError:
    HAS_DEPS=False
...
TWIC_REF_NUM=920; TWIC_REF_DATE=date(2012,6,25)
...
def day_names(): return [t(k) for k in DAY_KEYS]
def day_code_from_index(i): return DAY_CODES[i]

# =====================================================================
# HELPERS
# =====================================================================
def twic_date(num): return TWIC_REF_DATE+timedelta(days=(num-TWIC_REF_NUM)*7)
def twic_year(num): return twic_date(num).year
def first_twic_of_year(year):
    n=TWIC_REF_NUM+int((date(year,1,1)-TWIC_REF_DATE).days/7)
    while twic_date(n).year<year: n+=1
    return n
def latest_possible_twic(): return TWIC_REF_NUM+(date.today()-TWIC_REF_DATE).days//7

def estimate_duration(count):
    seconds=count*15
    if seconds<60: return "~<1 min"
    elif seconds<3600:
        m=seconds//60; return f"~{m} min"
    else:
        h=seconds/3600
        if h<2: return f"~{int(h)}h{int((h%1)*60):02d}"
        else: return f"~{h:.0f}h"

# =====================================================================
# ENGINE (unchanged logic, translated log messages)
# =====================================================================
class TWICEngine:
    def __init__(self,root_dir,start_twic=1313,end_twic=None,
                 elo_min=0,require_both=False,callback=None):
        self.root_dir=Path(root_dir); self.start_twic=start_twic
        self.end_twic=end_twic; self.elo_min=elo_min
        self.require_both=require_both
        self.cb=callback or (lambda*a:None); self.cancelled=False
        self.actual_last=start_twic-1

    def cancel(self): self.cancelled=True
    def year_dir(self,y):
        d=self.root_dir/f"TWIC {y}"; d.mkdir(parents=True,exist_ok=True)
        (d/"weekly_clean").mkdir(exist_ok=True); (d/"raw_downloads").mkdir(exist_ok=True)
        return d
    def state_file(self): return self.root_dir/"twic_state.json"
    def hashes_file(self,y): return self.year_dir(y)/"seen_hashes.txt"
    def annual_pgn(self,y): return self.year_dir(y)/f"TWIC_{y}_cumulative.pgn"
    def weekly_pgn(self,y,n): return self.year_dir(y)/"weekly_clean"/f"twic{n}_clean.pgn"
    def raw_zip(self,y,n): return self.year_dir(y)/"raw_downloads"/f"twic{n}g.zip"
    def config_file(self): return self.root_dir/"twic_config.json"

    def load_state(self):
        sf=self.state_file()
        return json.loads(sf.read_text(encoding="utf-8")) if sf.exists() else {"last_twic":self.start_twic-1}
    def save_state(self,state):
        tmp=self.state_file().with_suffix(".json.tmp")
        tmp.write_text(json.dumps(state,indent=2),encoding="utf-8"); tmp.replace(self.state_file())
    def load_hashes(self,y):
        f=self.hashes_file(y); return set(f.read_text(encoding="utf-8").splitlines()) if f.exists() else set()
    def save_hashes(self,y,h):
        f=self.hashes_file(y); tmp=f.with_suffix(".txt.tmp")
        tmp.write_text("\n".join(sorted(h)),encoding="utf-8"); tmp.replace(f)
    def save_config(self):
        cfg={"root_dir":str(self.root_dir),"start_twic":self.start_twic,
             "elo_min":self.elo_min,"require_both":self.require_both,"lang":_LANG}
        cf=self.config_file(); tmp=cf.with_suffix(".json.tmp")
        tmp.write_text(json.dumps(cfg,indent=2),encoding="utf-8"); tmp.replace(cf)

    def extract_pgn(self,zb):
        with zipfile.ZipFile(io.BytesIO(zb)) as z:
            n=next((x for x in z.namelist() if x.lower().endswith(".pgn")),None)
            if not n: raise ValueError("No PGN"); return z.read(n).decode("utf-8",errors="replace")
    def detect_year(self,pt):
        pi=io.StringIO(pt); c={}
        for _ in range(100):
            g=chess.pgn.read_game(pi)
            if g is None: break
            d=g.headers.get("Date","")
            if len(d)>=4:
                try:
                    y=int(d[:4])
                    if 1990<=y<=2100: c[y]=c.get(y,0)+1
                except ValueError: pass
        return max(c,key=c.get) if c else datetime.now().year
    def game_hash(self,g):
        h=g.headers
        return hashlib.sha1("|".join([h.get("White",""),h.get("Black",""),h.get("Date",""),
            h.get("Event",""),h.get("Round",""),
            " ".join(m.uci() for m in g.mainline_moves())]).encode()).hexdigest()
    def dedupe(self,pt,seen):
        pi=io.StringIO(pt); cl,nh,nn,nd=[],set(),0,0
        while True:
            try: g=chess.pgn.read_game(pi)
            except: continue
            if g is None: break
            v=g.headers.get("Variant","").strip().lower()
            if v not in ACCEPT_VARIANTS: continue
            gh=self.game_hash(g)
            if gh in seen or gh in nh: nd+=1; continue
            nh.add(gh); nn+=1
            try: cl.append(str(g))
            except: pass
        return "\n\n".join(cl)+"\n",nn,nd,nh
    def write_atomic(self,p,c):
        tmp=p.with_suffix(p.suffix+".tmp"); tmp.write_text(c,encoding="utf-8"); tmp.replace(p)
    def append_atomic(self,p,c):
        e=p.read_text(encoding="utf-8") if p.exists() else ""; self.write_atomic(p,e+c)

    def process_one(self,num):
        self.cb("log",f"TWIC {num}...")
        try: r=requests.get(TWIC_URL.format(num=num),headers={"User-Agent":USER_AGENT},timeout=60)
        except requests.RequestException as e: self.cb("log",f"  Network: {e}"); return False
        if r.status_code==404: self.cb("log",f"  404"); return False
        if r.status_code!=200: self.cb("log",f"  HTTP {r.status_code}"); return False
        try: pt=self.extract_pgn(r.content)
        except Exception as e: self.cb("log",f"  Error: {e}"); return False
        y=self.detect_year(pt); self.cb("log",f"  → {y}")
        self.year_dir(y); self.raw_zip(y,num).write_bytes(r.content)
        seen=self.load_hashes(y); cp,nn,nd,nh=self.dedupe(pt,seen)
        self.cb("log",f"  +{nn} new, -{nd} dup"); self.cb("games",nn)
        if nn>0:
            self.write_atomic(self.weekly_pgn(y,num),cp)
            self.append_atomic(self.annual_pgn(y),cp)
        seen|=nh; self.save_hashes(y,seen); return True

    def run_download(self):
        self.root_dir.mkdir(parents=True,exist_ok=True); self.save_config()
        state=self.load_state(); num=max(state["last_twic"]+1,self.start_twic); misses=0
        while not self.cancelled:
            if self.end_twic is not None and num>self.end_twic: break
            if self.end_twic is None and misses>=3: break
            ok=self.process_one(num)
            if ok: self.actual_last=num; state["last_twic"]=num; self.save_state(state); misses=0
            else: misses+=1
            self.cb("progress",num); num+=1
        self.cb("done",self.actual_last)

    def run_filter(self):
        if self.elo_min<=0: self.cb("filter_done",None); return
        self.cb("log",f"\nElo >= {self.elo_min}...")
        out=self.root_dir/f"TWIC_elite_{self.elo_min}+.pgn"
        pgns=sorted(self.root_dir.glob("TWIC */TWIC_*_cumulative.pgn"))
        if not pgns: self.cb("filter_done",None); return
        tk_=0
        with open(out,"w",encoding="utf-8") as o:
            for p in pgns:
                if self.cancelled: break
                self.cb("log",f"  {p.parent.name}"); nk=0
                with open(p,"r",encoding="utf-8",errors="replace") as f:
                    while True:
                        try: g=chess.pgn.read_game(f)
                        except: continue
                        if g is None: break
                        v=g.headers.get("Variant","").strip().lower()
                        if v not in ACCEPT_VARIANTS: continue
                        try: we=int(g.headers.get("WhiteElo","0")); be=int(g.headers.get("BlackElo","0"))
                        except: continue
                        ok=(we>=self.elo_min and be>=self.elo_min) if self.require_both else (we>=self.elo_min or be>=self.elo_min)
                        if not ok: continue
                        try: o.write(str(g)); o.write("\n\n"); nk+=1
                        except: pass
                o.flush(); self.cb("log",f"    → {nk}"); tk_+=nk
        mb=out.stat().st_size/1024/1024
        self.cb("log",f"\n{out.name} ({mb:.1f} Mo, {tk_})"); self.cb("filter_done",str(out))

    # ── Scheduled task (cross-platform) ──
    def create_schedule(self,day="WED",hour="12:00"):
        exe=sys.executable; cfg=str(self.config_file())
        self.cb("log",f"\nScheduled task...")
        if SYSTEM=="Windows":
            cmds=[f'schtasks /Delete /TN "TWIC Auto Downloader" /F 2>nul',
                  f'schtasks /Create /TN "TWIC Auto Downloader" /TR "\"{exe}\" --background \"{cfg}\"" /SC WEEKLY /D {day} /ST {hour} /RL LIMITED /F']
...
<string>{exe}</string><string>--background</string><string>{cfg}</string>
</array>
<key>StartCalendarInterval</key><dict>
<key>Weekday</key><integer>{day_map.get(day,3)}</integer>
<key>Hour</key><integer>{int(h)}</integer>
<key>Minute</key><integer>{int(m)}</integer>
</dict></dict></plist>"""
...
class WizardApp:
    def __init__(self):
        self.root=tk.Tk(); self.root.title(f"{APP_NAME} v{APP_VERSION}")
        self.root.geometry("650x540"); self.root.resizable(False,False)
        try:
        ...
        self.schedule_day_idx=tk.IntVar(value=2)  # Wednesday
        self.schedule_hour=tk.StringVar(value="12:00")

        self.container=tk.Frame(self.root)
        self.container.pack(fill="both",expand=True,padx=30,pady=10)
        sep=ttk.Separator(self.root,orient="horizontal"); sep.pack(fill="x",padx=20)
        self.nav=tk.Frame(self.root); self.nav.pack(fill="x",padx=30,pady=10)
        ...
        # Pages: 0=lang, 1=welcome, 2=folder, 3=range, 4=elite, 5=schedule, 6=progress
        self.pages=[self._page_lang,self._page_welcome,self._page_folder,
                    self._page_range,self._page_elite,self._page_schedule,self._page_progress]
        self.page_idx=0; self.engine=None; self.total_games=0
        self._show_page(0)

    def _guess_dir(self):
        for b in [Path.home()/"Documents",Path.home()/"OneDrive"/"Documents"]:
            d=b/"ChessBase"/"Bases"
            if d.exists(): return str(d/"The Week In Chess (TWIC)")
    ...
    def _show_page(self,idx):
        self.page_idx=idx; self._clear(); self.pages[idx](); self._update_nav()

    def next_page(self):
        if self.page_idx==0:
            pass  # lang already set by button click
        if self.page_idx==2:
            if not self.target_dir.get().strip():
                messagebox.showwarning("",t("folder_required")); return
        if self.page_idx==3 and self.range_mode.get()=="between":
            if self.start_twic.get()>self.end_twic.get():
                messagebox.showwarning("",t("range_invalid")); return
        if self.page_idx==4 and self.enable_elite.get():
            try: elo=int(self.elo_min.get())
            except ValueError: messagebox.showwarning("",t("elo_invalid")); return
            if elo<1000:
                if not messagebox.askyesno(t("elo_low_title"),t("elo_low_msg",elo=elo)): return
            elif elo>2900:
                messagebox.showwarning(t("elo_high_title"),t("elo_high_msg",elo=elo)); return
        if self.page_idx==5:
            self._show_page(6); self.root.update(); self._start_engine(); return
        if self.page_idx<len(self.pages)-1: self._show_page(self.page_idx+1)

    def prev_page(self):
        if self.page_idx>1: self._show_page(self.page_idx-1)
    ...
    def _select_lang(self,code):
        set_lang(code); self._show_page(1)
    ...
    def _page_folder(self):
        ...
        row=tk.Frame(f); row.pack(pady=15,anchor="center")
    ...
    def _page_range(self):
        ...
        mf=tk.Frame(f); mf.pack(anchor="center",pady=5)
        tk.Radiobutton(mf,text=t("range_from"),variable=self.range_mode,value="from",
                       font=("Segoe UI",10),command=self._update_range_ui).pack(anchor="w")
        tk.Radiobutton(mf,text=t("range_between"),variable=self.range_mode,value="between",
                       font=("Segoe UI",10),command=self._update_range_ui).pack(anchor="w")

        tk.Label(f,text=t("range_start"),font=("Segoe UI",9,"bold")).pack(pady=(8,0),anchor="center")
        self.lbl_start=tk.Label(f,text="",font=("Segoe UI",10,"bold")); self.lbl_start.pack(anchor="center")
        mx=latest_possible_twic()
        self.slider_start=tk.Scale(f,from_=920,to=mx,orient="horizontal",variable=self.start_twic,
                                   length=550,showvalue=False,command=self._update_range_labels)
        ...
        self.lbl_end=tk.Label(f,text="",font=("Segoe UI",10,"bold")); self.lbl_end.pack(anchor="center")
        self.slider_end=tk.Scale(f,from_=920,to=mx,orient="horizontal",variable=self.end_twic,
                                 length=550,showvalue=False,command=self._update_range_labels)
        self.slider_end.pack(anchor="center")

        tk.Label(f,text=t("range_shortcuts"),font=("Segoe UI",9)).pack(pady=(5,2),anchor="center")
        cy=date.today().year; years=list(range(2012,cy+1)); mid=(len(years)+1)//2
        for ry in[years[:mid],years[mid:]]:
            row=tk.Frame(f); row.pack(anchor="center",pady=1)
            for y in ry:
    ...
    def _update_range_ui(self):
        show_end=self.range_mode.get()=="between"
        if show_end:
            self.slider_start.pack_forget(); self.lbl_end_title.pack_forget()
            self.lbl_end.pack_forget(); self.slider_end.pack_forget(); self.lbl_estimate.pack_forget()
            self.slider_start.pack(anchor="center")
            self.lbl_end_title.pack(pady=(5,0),anchor="center"); self.lbl_end.pack(anchor="center")
            self.slider_end.pack(anchor="center"); self.lbl_estimate.pack(pady=(5,0),anchor="center")
        else:
            self.lbl_end_title.pack_forget(); self.lbl_end.pack_forget(); self.slider_end.pack_forget()
            self.end_twic.set(latest_possible_twic())
        self._update_range_labels(None)

    def _update_range_labels(self,_):
        s=self.start_twic.get(); ds=twic_date(s)
        self.lbl_start.config(text=f"TWIC n°{s}  —  {ds.strftime('%d/%m/%Y')}")
        if self.range_mode.get()=="between":
            e=self.end_twic.get(); de=twic_date(e)
            self.lbl_end.config(text=f"TWIC n°{e}  —  {de.strftime('%d/%m/%Y')}")
            count=max(0,e-s+1)
        else: count=latest_possible_twic()-s+1
    ...
    def _page_elite(self):
        ...
        self.elite_frame=tk.Frame(f); self.elite_frame.pack(anchor="center")
        r1=tk.Frame(self.elite_frame); r1.pack(pady=5,anchor="center")
        tk.Label(r1,text=t("elite_elo_label"),font=("Segoe UI",10)).pack(side="left")
        tk.Entry(r1,textvariable=self.elo_min,width=6,font=("Segoe UI",10),justify="center").pack(side="left",padx=5)
        tk.Label(r1,text="(FIDE: 1000–2882)",font=("Segoe UI",8,"italic"),fg="gray").pack(side="left",padx=5)
        r2=tk.Frame(self.elite_frame); r2.pack(pady=5,anchor="center")
        tk.Label(r2,text=t("elite_criteria"),font=("Segoe UI",10)).pack(side="left")
        tk.Radiobutton(r2,text=t("elite_at_least_one"),variable=self.require_both,value=False,font=("Segoe UI",10)).pack(side="left",padx=8)
        tk.Radiobutton(r2,text=t("elite_both"),variable=self.require_both,value=True,font=("Segoe UI",10)).pack(side="left",padx=8)
        self._toggle_elite()

    def _toggle_elite(self):
        st="normal" if self.enable_elite.get() else "disabled"
    ...
    def _page_schedule(self):
        ...
        self.sched_frame=tk.Frame(f); self.sched_frame.pack(anchor="center")
        row=tk.Frame(self.sched_frame); row.pack(anchor="center",pady=5)
        ...
        self.day_combo.current(self.schedule_day_idx.get()); self.day_combo.pack(side="left",padx=5)
    ...
    def _toggle_sched(self):
        st="normal" if self.enable_schedule.get() else "disabled"
    ...
    def _update_summary(self):
        ...
        if self.enable_elite.get():
            m=t("elite_both") if self.require_both.get() else t("elite_at_least_one")
            lines.append(f"⭐  Elo ≥ {self.elo_min.get()} ({m})")
        if self.enable_schedule.get():
            di=self.day_combo.current(); dn=day_names()[di] if di>=0 else "?"
            lines.append(f"🔄  {dn} {self.schedule_hour.get()}")
    ...
    def _page_progress(self):
        ...
        self.progress=ttk.Progressbar(f,length=570,mode="determinate"); self.progress.pack(pady=5,anchor="center")
        self.lbl_status=tk.Label(f,text=t("progress_init"),font=("Segoe UI",10)); self.lbl_status.pack(anchor="center")
        self.lbl_games=tk.Label(f,text=f"{t('progress_games')} 0",font=("Segoe UI",9)); self.lbl_games.pack(anchor="center")
        self.log_text=tk.Text(f,height=12,font=("Consolas",9),state="disabled",bg="#1e1e1e",fg="#d4d4d4",wrap="word")
    ...
    def _start_engine(self):
        ...
        self.progress["maximum"]=max(1,target_end-start+1); self.progress["value"]=0; self.total_games=0
        elo=0
        if self.enable_elite.get():
            try: elo=int(self.elo_min.get())
            except: elo=0
        self.engine=TWICEngine(root_dir=self.target_dir.get(),start_twic=start,end_twic=end,
            elo_min=elo,require_both=self.require_both.get(),
            callback=lambda e,d:self.root.after(0,self._on_event,e,d))
        threading.Thread(target=self._run_pipeline,daemon=True).start()

    def _run_pipeline(self):
        self.engine.run_download()
        if self.engine.elo_min>0 and not self.engine.cancelled: self.engine.run_filter()
        if self.enable_schedule.get() and not self.engine.cancelled:
            di=self.day_combo.current(); dc=day_code_from_index(di) if di>=0 else "WED"
            self.engine.create_schedule(day=dc,hour=self.schedule_hour.get())
        if not self.engine.cancelled: self.engine.cb("all_done",None)

    def _on_event(self,evt,data):
        if evt=="log": self._log(str(data))
        elif evt=="progress":
            v=data-self.engine.start_twic+1
            self.progress["value"]=min(v,self.progress["maximum"])
            ...
            if self.engine.elo_min<=0 and not self.enable_schedule.get(): self._finish()
        elif evt=="filter_done":
            if not self.enable_schedule.get(): self._finish()
        elif evt=="all_done": self._finish()
    ...
    def _finish(self):
        ...
        if self.engine.elo_min>0: self._log(f"⭐ TWIC_elite_{self.engine.elo_min}+.pgn")
        if self.enable_schedule.get():
            di=self.day_combo.current(); dn=day_names()[di] if di>=0 else "?"
            self._log(f"🔄 {dn} {self.schedule_hour.get()}")
        self._log(f"\n{t('ctg_instructions')}")
        self._log("1. ChessBase → .cbh"); self._log("2. Ctrl+A → .ctg")
...
def run_background(config_path):
    ...
    if "lang" in cfg: set_lang(cfg["lang"])
    e=TWICEngine(root_dir=cfg["root_dir"],start_twic=cfg.get("start_twic",920),
                 elo_min=cfg.get("elo_min",0),require_both=cfg.get("require_both",False),
                 callback=lambda ev,d:logging.info(f"{d}") if ev=="log" else None)
    e.run_download()
    if cfg.get("elo_min",0)>0: e.run_filter()

def main():
    if "--background" in sys.argv:
        i=sys.argv.index("--background")
        if i+1<len(sys.argv): run_background(sys.argv[i+1])
        return
    if not HAS_TK: print("tkinter unavailable"); sys.exit(1)
    if not HAS_DEPS:
        if HAS_TK:
            r=tk.Tk(); r.withdraw()
            messagebox.showerror("","pip install requests python-chess"); r.destroy()
        sys.exit(1)
    WizardApp().run()
